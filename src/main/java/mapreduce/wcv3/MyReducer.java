...
public class MyReducer extends Reducer<MyKey, Text, Text, Text> {
	/**
	 * Receives output from the partitioners 
	 */
	...
	protected void reduce(MyKey key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		if (key.tag.equals("TF")) {
			for (Text v : values) {
				// System.out.println("key:"+key.term+" : docid :"+key.docid+ " : "+"termfre:
				// "+key.frequency);
				output_files.write("Termfrequency", new Text(key.term + " : " + key.docid + " : "),
						new Text(key.frequency));
